...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_SPARSE_MATRIX(S); // smoothing matrix
  ...
  nll -= Type(0.5) * S.cols() * log_lambda_dive - 0.5 * lambda_dive * GMRF(S).Quadform(s_dive); 
  nll -= Type(0.5) * S.cols() * log_lambda_surf - 0.5 * lambda_surf * GMRF(S).Quadform(s_surf); 
